...
| 5 | The latent budget ends; `e_1` goes through the Language Model and the LM Head samples `x_2`. | Switch back to normal LM Head sampling |
| 6 | `x_2` goes through the LM Head and samples a second `<bot>`. | Discrete mode continues until another `<bot>` is sampled |
| 7 | The second `<bot>` starts a longer latent segment; `h_2` is projected to `e_2`. | Longer latent segment: `h_2` to `e_2` |
| 8 | `e_2` feeds back; the Language Model produces `h_3`; the projector predicts `e_3`. | `e_2` feeds back; `h_3` is projected to `e_3` |
| 9 | `e_3` feeds back; the Language Model produces `h_4`; the projector predicts `e_4`. | `e_3` feeds back; `h_4` is projected to `e_4` |
| 10 | `e_4` exits latent mode, then the final LM Head samples `a_1`, `a_2`, `a_3`, and `a_4` one at a time. | The LM Head samples answer tokens like normal, one at a time |
